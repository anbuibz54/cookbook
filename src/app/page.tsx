    ...
    <PageTransition>
      <div className="mx-auto flex min-h-dvh w-full max-w-md flex-col gap-5 px-[18px] pt-6 pb-32">
        <header className="flex items-center justify-between">
          ...
          <Link
            href="/recipes/new" transitionTypes={['nav-forward']}
        ...
        {tags.length > 0 ? (
          <div className="grid grid-cols-2 gap-3">
            {tags.map((tag, i) => (
              <Link
                key={tag.tag}
                href={`/recipes?tag=${encodeURIComponent(tag.tag)}`} transitionTypes={['tab']}
        ...
        <div className="flex items-baseline justify-between">
          ...
          <Link href="/recipes" transitionTypes={['tab']} className="text-[13px] text-muted hover:text-ink">
            Xem tất cả
          </Link>
        </div>
        ...
        {recent.length === 0 ? (
          ...
          <ul className="flex flex-col gap-3">
            {recent.map((recipe, i) => (
              <li key={recipe.id}>
                <Link
                  href={`/recipes/${recipe.id}`} transitionTypes={['nav-forward']}
