...
/**
 * What a recipe would take out of the pantry — the list the "vừa nấu xong"
 * screen shows for confirmation. Nothing is deducted until the user says so.
 */
export async function pantryUsedBy(
  db: Db,
  userId: string,
  recipeId: string,
): Promise<{ item: PantryItem; needGrams: number | null; ingredient: string }[]> {
  const [pantry, lines] = await Promise.all([
    listPantry(db, userId),
    db
      .select({
  ...
  const used: { item: PantryItem; needGrams: number | null; ingredient: string }[] = []
  for (const line of lines) {
    const key = matchKey(line.name)
    const item =
      pantry.find((p) => p.foodId != null && p.foodId === line.foodId) ??
      pantry.find((p) => covers(p.matchKey, key))
    if (item && !used.some((u) => u.item.id === item.id)) {
      used.push({ item, needGrams: line.grams, ingredient: line.name })
    }
  }
  return used
}
...
/**
 * Take used ingredients out of the pantry.
 *
 * A line is subtracted when both sides are weighed and something is left over
 * (300 g thịt, recipe took 200 g → 100 g stays); otherwise it is removed.
 * Guessing "half of a bó hành" would be worse than either.
 */
export async function deductFromPantry(
  db: Db,
  userId: string,
  deductions: { itemId: string; grams: number | null }[],
): Promise<{ reduced: number; removed: number }> {
  if (deductions.length === 0) return { reduced: 0, removed: 0 }

  const items = await db
    .select()
    .from(pantryItems)
    .where(and(eq(pantryItems.userId, userId), inArray(pantryItems.id, deductions.map((d) => d.itemId))))

  const wanted = new Map(deductions.map((d) => [d.itemId, d.grams]))
  const toRemove: string[] = []
  let reduced = 0

  for (const item of items) {
    const need = wanted.get(item.id) ?? null
    const left = item.grams != null && need != null ? item.grams - need : 0

    if (left > 0) {
      const ratio = item.quantity != null && item.grams ? left / item.grams : null
      await db
        .update(pantryItems)
        .set({
          grams: left,
          quantity: ratio != null && item.quantity != null ? item.quantity * ratio : item.quantity,
          quantityMax: ratio != null && item.quantityMax != null ? item.quantityMax * ratio : item.quantityMax,
          updatedAt: new Date(),
        })
  ...
  if (toRemove.length > 0) {
    await db
      .delete(pantryItems)
      .where(and(eq(pantryItems.userId, userId), inArray(pantryItems.id, toRemove)))
  }

  return { reduced, removed: toRemove.length }
}
